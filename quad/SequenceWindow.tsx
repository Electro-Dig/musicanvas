...
export function SequenceWindow({current,previous,phase,playing,padId,selected,onSelect}:{current:SequenceRound;previous:SequenceRound|null;phase:number;playing:boolean;padId:string;selected:string|null;onSelect:(id:string)=>void}) {
 ...
 const changes=sequenceChanges(current,previous,tr);
 const hitLabel=(h:SequenceHit)=>tr("{0} · 第 {1} 步{2}", h.name, h.step+1, h.muted?tr(" · 静音连接点"):'');
 const plot=(round:SequenceRound,old:boolean)=><div className="sequence-plot" style={{height:rows*20+28,minWidth:Math.max(700,steps*42)}}>
  ...
  {round.hits.filter(h=>!old||round.played.includes(h.nodeId)).map(h=><button key={h.nodeId} title={hitLabel(h)} aria-label={`${old?tr("上一轮"):tr("本轮")} ${hitLabel(h)}`} className={`sequence-hit ${h.muted?'is-muted':''} ${selected===h.nodeId?'is-selected':''} ${round.played.includes(h.nodeId)?'is-played':''}`} style={{left:`calc(52px + (100% - 52px) * ${h.step/steps})`,top:h.muted?rows*20+3:(hi-h.midi)*20,width:`calc((100% - 52px) / ${steps} - 4px)`}} onClick={()=>onSelect(h.nodeId)} onMouseEnter={()=>setInspect(hitLabel(h))} onMouseLeave={()=>setInspect(null)}>{h.muted?'◇':h.name}</button>)}
  {!old&&playing&&<div className="sequence-playhead" style={{left:`calc(52px + (100% - 52px) * ${phase*current.steps/steps})`}}/>}
  <span className="sequence-rest-label" style={{top:rows*20+3}}>{tr("连接")}</span>
 </div>;
 return <section className={`sequence-window ${open?'':'is-closed'}`} aria-label={tr("音序对照窗")}>
  <header><div><b>{tr("音序 / PAD")}{padId}</b><span>{current.steps} {tr("步 · 触发点 × 音高")}</span></div><div><button onClick={()=>setOpen(!open)}>{open?tr("收起音高细节"):tr("展开音高细节")}</button></div></header>
  <div className="sequence-summary">{inspect??(previous?(changes.length?changes.join(' · '):tr("本轮计划与上一轮触发顺序相同")):tr("播放两轮，查看音序变化"))}<small>{tr("实心：已触发 空心：待触发 ◇：静音连接 同列：同时触发")}</small></div>
  ...
  {open&&<><div className="sequence-scroll"><div className="sequence-panels"><div><h3>{tr("本轮计划")}<span>{playing?tr("播放中"):tr("未播放 / 停止")} · #{current.cycle+1}　{tr("｜ 上一轮实际触发：")}{previous?tr("#{0}（虚线）", previous.cycle+1):tr("等待记录")}</span></h3><div className="sequence-overlay">{plot(current,false)}{previous&&<div className="sequence-history">{plot(previous,true)}</div>}</div></div></div></div>
  <footer>{tr("仅显示乐句边界内的触发。横向读顺序，纵向读音高。块宽只标触发点，不代表声音时长；点击音名选中画布节点。历史记录为到时调度事件，不是录音检测。")}</footer></>}
 </section>;
}
