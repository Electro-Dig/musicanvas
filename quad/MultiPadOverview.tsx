...
export default function MultiPadOverview({pads, progress}: {pads: QuadLilyPad[]; progress: PhaseComparisonPad[]}) {
  ...
  return <section className="phase-comparison" aria-label="四轨周期总览">
    <p>共同时间刻度 · 每行从自己的乐句起点计时</p>
    <div className="multi-pad-axis"><span>0 秒</span><span>{(span/2000).toFixed(2)}</span><span>{(span/1000).toFixed(2)} 秒</span></div>
    {pads.map((p,i)=>{const state=progress.find(s=>s.id===p.id);const duration=getPadCycleDurationMs(p);return <div className="multi-pad-row" key={p.id} style={{'--track-color':`var(--quad-pad-${p.id.toLowerCase()})`} as React.CSSProperties}>
      <header><strong>{p.id}</strong><span>{state?.bpm} BPM · {Math.round(duration / (p.intervalMs / 4))} 步</span><small>{p.playing?'播放中':'已暂停'}</small></header>
      <svg viewBox="0 0 280 80" role="img" aria-label={`Pad ${p.id} 乐句事件与播放位置`}>
        ...
        {cycles[i].nodes.filter(n=>n.status==='active').map(n=><g key={n.nodeId}><circle cx={8+264*(n.offsetMs??0)/span} cy="40" r="3.5" fill="var(--track-color)"/><title>{n.nodeId} · {((n.offsetMs??0)/1000).toFixed(3)} 秒</title></g>)}
    ...
    <p>圆点表示本轮可到达的事件时点，竖线表示当前进度。暂停时保留位置；详细传播关系见“当前轨”。</p>
  </section>;
}
